            ...
            if is_number(command[1]):
                ...
                if is_valid_duration(duration):
                    ...
                    if user is not None:
                        ...
                        self.storage.settings["guilds"][guild_id]["banned_users"][str(user_id)]["normal_duration"] = command[2]
                        await self.storage.write_settings_file_to_disk()
                        # Message the channel
                        await message.channel.send(f"**Temporarily banned user:** `{user.name}` **for:** `{command[2]}`**. Reason:** `{reason}`")
